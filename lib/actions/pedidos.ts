...
import { podeOperarConferencia, podeOperarExpedicao } from "@/lib/policies"
import { z } from "zod"
import type {
  Pedido,
  ItemPedido,
...
/**
 * 4. actionIniciarSeparacao
 * Inicia separação, atribui separador
 */
export async function actionIniciarSeparacao(pedidoId: string) {
  const session = await auth()
  if (!session?.user?.empresaId || !session.user.id) {
    return { ok: false, error: "Não autenticado" }
  }
  ...
  try {
    ...
    if (pedido.status !== "RESERVADO") {
      return { ok: false, error: "Pedido não está em status RESERVADO" }
    }

    const pedidoFormatado = formatarPedido(pedido)
    if (!podeOperarSeparacao(pedidoFormatado, session.user)) {
      return { ok: false, error: "Não é possível operar separação neste pedido" }
    }
...
/**
 * 5. actionMarcarItemSeparado
 * Marca um item como separado
 */
export async function actionMarcarItemSeparado(pedidoId: string, itemId: string) {
  const session = await auth()
  if (!session?.user?.empresaId || !session.user.id) {
    return { ok: false, error: "Não autenticado" }
  }
  ...
  try {
    ...
    if (item.status === "PENDENTE_ESTOQUE" || item.status === "CANCELADO") {
      return { ok: false, error: "Item não pode ser marcado como separado" }
    }
    ...
    return { ok: false, error: "Erro ao marcar item separado" }
  }
}

/**
 * 6. actionConcluirSeparacao
 * Conclui separação, move para conferência
 */
export async function actionConcluirSeparacao(pedidoId: string) {
  const session = await auth()
  if (!session?.user?.empresaId || !session.user.id) {
    return { ok: false, error: "Não autenticado" }
  }
  ...
  try {
    ...
    // Confirma que todos os itens separáveis estão SEPARADO
    const itensSeparaveis = pedido.itens.filter(
      (item) => item.status !== "PENDENTE_ESTOQUE" && item.status !== "CANCELADO"
    )

    const todosSeparados = itensSeparaveis.length > 0 && itensSeparaveis.every((item) => item.status === "SEPARADO")
    if (!todosSeparados) {
      return { ok: false, error: "Nem todos os itens foram separados" }
    }
    ...
    return { ok: false, error: "Erro ao concluir separação" }
  }
}

/**
 * 7. actionConfirmarConferencia
 * Confirma conferência, move para CONFERIDO
 */
export async function actionConfirmarConferencia(pedidoId: string) {
  const session = await auth()
  if (!session?.user?.empresaId || !session.user.id) {
    return { ok: false, error: "Não autenticado" }
  }

  if (!podeOperarConferencia(session.user)) {
  ...
  try {
    ...
    await prisma.pedidoEvento.create({
      data: {
        ...
        descricao: "Conferência concluída.",
        usuarioId: session.user.id,
      },
    })
    ...
    return { ok: false, error: "Erro ao confirmar conferência" }
  }
}

/**
 * 8. actionRegistrarDivergenciaConferencia
 * Registra divergência na conferência
 */
export async function actionRegistrarDivergenciaConferencia(pedidoId: string, descricao: string) {
  const session = await auth()
  if (!session?.user?.empresaId || !session.user.id) {
    return { ok: false, error: "Não autenticado" }
  }
  ...
  if (!descricao.trim()) {
    return { ok: false, error: "Informe a descrição da divergência" }
  }
  ...
  try {
    ...
    await prisma.pedido.update({
      where: { id: pedidoId },
      data: { pendencia: "DIVERGENCIA_CONFERENCIA" },
    })
    ...
    return { ok: false, error: "Erro ao registrar divergência" }
  }
}
